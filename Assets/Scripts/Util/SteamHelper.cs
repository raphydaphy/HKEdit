...
namespace Util {
    public static class SteamHelper {
        ...
        public static string GetHollowKnightDataPath() {
            string gamePath = SteamHelper.GetHollowKnightPath();

            if (gamePath == "" || !Directory.Exists(gamePath)) {
                EditorUtility.DisplayDialog("HKEdit",
                    "Could not find Steam path. If you've moved your Steam directory this could be why. Contact nes.",
                    "OK");
                return null;
            }
        ...
        private static string FindSteamGamePath(int appid, string gameName) {
            if (ReadRegistrySafe("Software\\Valve\\Steam", "SteamPath") == null) {
                EditorUtility.DisplayDialog("HKEdit",
                    "You either don't have steam installed or your registry variable isn't set.", "OK");
                return "";
            }
            ...
            if (path == null) {
                EditorUtility.DisplayDialog("HKEdit",
                    "It appears you don't have " + gameName +
                    " installed anywhere. The game files are needed to find the game.", "OK");
            }
            else {
                return path;
            }
